...
p <- dplyr::filter(mc, Pathway=="KEGG_PATHWAYS_IN_CANCER")
  p$numNcvs <- as.numeric(paste0(p$numNcvs))
  p$numGenes <- as.numeric(paste0(p$numGenes))
  glimpse(p)  
 
nk <- as.numeric(paste0(df$numNcvs[df$Pathway=="KEGG_PATHWAYS_IN_CANCER"]))
 
hist(p$numNcvs,breaks = 20, xlim = c(0,nk+5));abline(v=nk, col="red")
```
